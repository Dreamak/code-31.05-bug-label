...
@obj_mon_application.route("/droit_afficher/<int:id_droit_sel>", methods=['GET', 'POST'])
def droits_afficher(id_droit_sel):
    if request.method == "GET":
        try:
            ...
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                strsql_droits_afficher_data = """SELECT Id_personne, Nom_personne, Prenom_personne, Date_naissance_personne, Adresse_mail_personne, MDP_personne,
                                                            GROUP_CONCAT(droit) as Droits FROM t_avoir_droit
                                                            RIGHT JOIN t_personne ON t_personne.id_personne = t_avoir_personne.Fk_personne
    ...
    return render_template("droits/droits_afficher.html", data=data_droits_afficher)
...
"""
    ...
    Récupère la liste de tous les genres du film sélectionné par le bouton "MODIFIER" de "droits_afficher.html"
...
def edit_droit_selected():
    ...
    return render_template("droits/droits_modifier_tags_dropbox.html",
                           ...
                           data_film_selected=data_droit_selected,
...
def droits_personnes_afficher_data(valeur_id_personne_selected_dict):
    ...
    try:
        ...
        strsql_personne_selected = """SELECT id_film, nom_film, duree_film, description_film, cover_link_film, date_sortie_film, GROUP_CONCAT(id_genre) as GenresFilms FROM t_genre_film
        ...
        strsql_droits_personnes_non_attribues = """SELECT id_genre, droit FROM t_genre WHERE id_genre not in(SELECT id_genre as idGenresFilms FROM t_genre_film
                                                    INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                                    INNER JOIN t_genre ON t_genre.id_genre = t_genre_film.fk_genre
                                                    WHERE id_film = %(value_id_film_selected)s)"""

        strsql_droits_personnes_attribues = """SELECT id_film, id_genre, droit FROM t_genre_film
                                            INNER JOIN t_film ON t_film.id_film = t_genre_film.fk_film
                                            INNER JOIN t_genre ON t_genre.id_genre = t_genre_film.fk_genre
                                            WHERE id_film = %(value_id_film_selected)s"""
